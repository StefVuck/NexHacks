...
**Important**: CSV data is generated by **firmware running in QEMU** (ARM emulator), demonstrating how real microcontrollers with Woodwide AI integration would work.

### The Process

1. **Firmware Generation**: Claude AI generates embedded C code for each sensor with CSV export logic
2. **CSV Integration**: Woodwide AI CSV templates are automatically injected into firmware
3. **Compilation**: Firmware is compiled to ARM binary using arm-none-eabi-gcc
4. **QEMU Simulation**: Binary runs in QEMU ARM emulator (simulated microcontroller hardware)
5. **CSV Output**: Running firmware outputs CSV data via serial/semihosting
6. **Data Capture**: CSV output is captured from QEMU and saved to files

This demonstrates exactly how real microcontrollers would generate and export CSV data using the Woodwide AI integration.
